...
- **PRODUCTION LAUNCH — put the client portal on a real subdomain (THE NEXT TASK).** The PL team can't use it with real clients until it's on `portal.propertylions.com.au` (proposed) running a Clerk **production** instance. Gated, mostly user-dashboard work:
  1. **Clerk prod instance** — create it (kills the "Development mode" banner + dev rate limits). Requires: Clerk DNS CNAMEs on `propertylions.com.au` (`clerk.`, `accounts.`, `clkmail.`…); its **own Google Cloud OAuth app** (the dev instance used Clerk's shared Google creds, prod needs yours); enable **Password + email code** (also unblocks the `/portal` reset flow).
  2. **Env swaps** — `pk_live_…` into Vercel (`VITE_CLERK_PUBLISHABLE_KEY`), `sk_live_…` into Convex `colorless-condor-502` (`CLERK_SECRET_KEY`), and update `convex/auth.config.ts` issuer/JWKS to the prod Clerk domain.
  3. **Vercel custom domain** — add `portal.propertylions.com.au` to the `oracle-app` project; DNS `CNAME → cname.vercel-dns.com`.
  4. **CODE (agent's part)** — `convex/team.ts` hardcodes the invite `redirect_url` to `oracle-psi-beryl.vercel.app`; make it env-driven for the new domain. (`ClientPortalLogin` already uses `window.location.origin` for `/sso-callback`, so that follows the domain automatically.)
  - Ownership: Clerk / Google Cloud / DNS steps are the **user's** dashboards; the agent does the code + env wiring + end-to-end verify. **DNS host for propertylions.com.au: ask the user.** Decide staff domain too (stay on current URL vs `app.propertylions.com.au`).
