...
- **Directory tab — BUILT, pushed, not yet deployed (2026-07-23).** Will's Jul-31 request: one searchable place for clients + agents + contractors/inspectors + solicitors + brokers, filter by category + state. New `contacts` table + `convex/contacts.ts` (CRUD, indexed); `src/pages/Directory.jsx` (aggregates live clients + the new contacts store) + `components/directory/ContactModal.jsx`; `/directory` route + Sidebar item. On `colorless-condor-502` (schema+functions pushed via `convex dev --once`). **TO GO LIVE:** `npx convex deploy --yes` (695) → `vercel --prod` → alias both domains. **Not yet visually verified behind CRM auth** (build/lint/authz-gate/query-guard all green).
- **Meeting backlog (from Will, cross-checked 2026-07-23 vs live).** Still unbuilt, roughly in Will's priority order: (1) **DD task-management / pipeline checklist** — task assignment, client-visible progress, contract important-dates, email reminders (Will's Jul-31 #1; he saw it in Allan's *other* system, not Oracle). (2) ~~Directory~~ ✅ built above. (3) **Building-grade field (Prime/A/B/C)** on comps — quick schema add. (4) **Comps summary view missing state/postcode** columns — display bug (data is in each record). (5) **Feaso Cashflow (10-yr model)** — the Cashflow sub-tab is a placeholder stub. (6) **AI-populated / downloadable FISO spreadsheet + Google-Sheets sandbox** (Jul-31). Deferred/agreed-non-essential: **map view for comps**, **Aerolytics API** (Will to re-evaluate vendor first). Provenance: `AG-ORACLE/Google Meet Notes/` (Jun-26, Jul-03, Jul-31 PDFs).
