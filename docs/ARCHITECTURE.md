...
| `comps` | Sales/lease comparables (`source`: property_lions / historical_import / arealytics / im_scan / comp_scan / …). **110 rows live**, all `property_lions` Industrial — the deliberate curated set. A ~262k bulk archive was loaded 6 Jul and **deliberately wiped 28 Jul** for accuracy; see STATE.md "comp archive history" before bulk-loading. |
...
- **Comps at scale** — the schema/UI are built for ~262k rows (browse via `usePaginatedQuery` infinite scroll + a `search_address` search index; a `source` dropdown defaults to Property Lions). The DB deliberately holds a small curated set (110 rows, Brisbane-area Industrial) — a bulk archive was loaded then wiped for accuracy on 28 Jul; see STATE.md "comp archive history". Feaso "suggested comps" matching: suburb-first, then widened to same state + asset type (`comps.suggestCompsForProperty`, `by_state_assetType_type` index). Browse filters (state / asset type / recency / size) are applied **server-side** — they must never move back to the client, which silently under-reports once the table is large.
